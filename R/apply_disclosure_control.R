...
apply_disclosure_control <- function(df) {
    ...
    df %>%
        dplyr::mutate(numerator = replace_and_round(.data$numerator),
                      denominator = replace_and_round(.data$denominator),
                      indicator = round(.data$numerator/.data$denominator, 4)
               )
